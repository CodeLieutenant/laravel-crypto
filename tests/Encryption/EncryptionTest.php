...
class EncryptionTest extends TestCase
{
    ...
    {
        self::assertTrue(SodiumEncryptor::supported($this->key, SodiumEncryptor::AES256CBC));

        if (sodium_crypto_aead_aes256gcm_is_available()) {
            self::assertTrue(SodiumEncryptor::supported($this->key, SodiumEncryptor::AES256GCM));
        }

        self::assertTrue(SodiumEncryptor::supported($this->key, SodiumEncryptor::XChaCha20Poly1305));
        self::assertFalse(SodiumEncryptor::supported($this->key, SodiumEncryptor::AES128CBC));
    ...
    public function test_supported_algorithms_with_crypt_facade(): void
    {
        ...
        self::assertTrue(Crypt::supported($this->key, SodiumEncryptor::AES256CBC));
        if (sodium_crypto_aead_aes256gcm_is_available()) {
            self::assertTrue(Crypt::supported($this->key, SodiumEncryptor::AES256GCM));
        }
        self::assertTrue(Crypt::supported($this->key, SodiumEncryptor::XChaCha20Poly1305));
        self::assertFalse(Crypt::supported($this->key, SodiumEncryptor::AES128CBC));
